...
export default function Home() {
  return (
    <main className="relative overflow-hidden pt-6 md:pt-12 max-h-screen overflow-y-hidden">
      <section>
      ...
      <section className="relative z-10 lg:pt-20 xl:pt-30">
        <div className="animate-marquee w-full">
          <div className="grid h-full w-[500vw] grid-cols-8 place-items-center gap-4 px-6 sm:w-[300vw] xl:w-[200vw]">
            {projects.map(({ img, href }, i) => {
              if ((i + 1) % 2 === 0) {
                return (
                  <Project className="-mb-40" img={img} href={href} key={i} />
                );
              } else {
                return <Project img={img} href={href} key={i} />;
              }
            })}
          </div>
        </div>
